...
<body class="bg-blue-50 min-h-screen" data-daily-goal="<?php echo $daily_goal; ?>">
    ...
    <div class="max-w-4xl mx-auto p-6 mt-6">
        ...
        <!-- Title -->
        <div class="mb-8">
            <h1 class="text-2xl font-bold text-gray-800">Your Hydration Dashboard</h1>
            <p class="text-gray-500 text-sm">Track your water intake and stay healthy.</p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-blue-100 flex items-center justify-between">
                <div>
                    <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Daily Goal</p>
                    <h2 class="text-3xl font-bold text-blue-600"><?php echo $daily_goal; ?> <span class="text-sm text-gray-400 font-normal">ml</span></h2>
                </div>
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-blue-100 flex items-center justify-between">
                <div>
                    <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Today's Intake</p>
                    <h2 class="text-3xl font-bold <?php echo ($total_intake >= $daily_goal) ? 'text-green-500' : 'text-slate-800'; ?>">
        ...
        <div class="flex flex-col items-center justify-center py-8">
            ...
            <div id="goal-success-banner" class="<?php echo ($total_intake >= $daily_goal) ? '' : 'hidden'; ?> mb-6 bg-green-500 text-white p-4 rounded-2xl shadow-lg shadow-green-200 animate-fade-in flex items-center justify-between">
                ...
                <i class="fa-solid fa-xmark cursor-pointer text-white/70 hover:text-white p-2" onclick="this.parentElement.classList.add('hidden')"></i>
            </div>
            ...
            <form action="<?php echo BASE_URL; ?>/actions/log_water.php" method="POST" class="ajax-form relative group">
                ...
                <div id="sip-tooltip" class="absolute 
                    ...
                    <div class="absolute w-3 h-3 bg-white border-blue-50 rotate-45 transform z-10
                ...
                <button type="submit" class="relative block transition-transform active:scale-95 duration-150 focus:outline-none">
                    <div class="w-16 h-8 mx-auto bg-blue-100 border-x-4 border-t-4 border-blue-200 rounded-t-xl translate-y-1"></div>

                    <div class="w-44 h-72 bg-white/50 backdrop-blur-sm border-4 border-blue-200 rounded-[3rem] relative overflow-hidden shadow-[0_20px_50px_-12px_rgba(59,130,246,0.3)]">
                        
                        <div id="water-level-fill" class="absolute bottom-0 w-full bg-blue-500/80 transition-all duration-1000 ease-in-out" 
                ...
                <div class="w-full max-w-md mx-auto mt-8 bg-white p-6 rounded-2xl shadow-sm border border-gray-100 text-center">
                    <div class="grid grid-cols-2 gap-4 w-full">
                        ...
                        <form action="<?php echo BASE_URL; ?>/actions/log_water.php" method="POST" class="ajax-form">
                            ...
                            <button type="submit" class="w-full flex flex-col items-center justify-center gap-2 p-4 rounded-2xl bg-blue-600 border border-blue-600 text-white shadow-lg shadow-blue-200 hover:bg-blue-700 hover:border-blue-700 hover:shadow-xl transition-all active:scale-95 h-full">
                        ...
                        <button onclick="openManualModal()" type="button" class="flex flex-col items-center justify-center gap-2 p-4 rounded-2xl bg-white border border-slate-200 text-slate-500 hover:text-blue-600 hover:border-blue-200 hover:shadow-md transition-all active:scale-95 h-full w-full">
            ...
            <?php else: ?>
                ...
                <div class="w-full max-w-md mx-auto p-6 bg-slate-50 border border-slate-200 rounded-2xl text-center text-slate-500">
                    <i class="fa-solid fa-calendar-check text-2xl mb-2 text-slate-400"></i>
                    <p>You are viewing a past log.</p>
                    <a href="dashboard.php" class="text-blue-600 font-bold text-sm hover:underline mt-2 inline-block">Return to Today</a>
                </div>
        ...
        <div class="w-full max-w-sm mx-auto mt-12">
            <h3 class="text-center font-hand text-xl text-slate-600 mb-4 font-bold">Today's Log</h3>
            ...
            <div class="flex items-center justify-between mb-6 px-4">
                ...
                <a href="dashboard.php?date=<?php echo $prev_date; ?>" class="w-10 h-10 flex items-center justify-center rounded-full bg-white border border-slate-200 text-slate-500 hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 transition shadow-sm">
                ...
                <?php if (!$is_today): ?>
                    <a href="dashboard.php?date=<?php echo $next_date; ?>" class="w-10 h-10 flex items-center justify-center rounded-full bg-white border border-slate-200 text-slate-500 hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 transition shadow-sm">
        ...
        <div id="manual-modal" class="fixed inset-0 z-50 hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            ...
            <div class="absolute inset-0 bg-slate-900/40 backdrop-blur-sm transition-opacity" onclick="closeManualModal()"></div>

            <div class="fixed inset-0 z-50 flex items-end justify-center md:items-center pointer-events-none">
                
                <div class="pointer-events-auto w-full md:w-96 bg-white rounded-t-[2rem] md:rounded-3xl shadow-2xl transform transition-all duration-300 ease-out p-8 pb-10 max-h-[90vh] overflow-y-auto">
                    ...
                    <form action="<?php echo BASE_URL; ?>/actions/log_water.php" method="POST" class="ajax-form" id="manual-form">
                        ...
                        <div class="relative mb-8 w-3/4 mx-auto">
                            ...
                            <span class="absolute -right-4 top-1/2 -translate-y-1/2 text-gray-400 font-medium text-lg pointer-events-none">ml</span>
                        </div>

                        <div class="flex justify-center gap-3 mb-8">
                            <button type="button" onclick="setAmount(150)" class="px-4 py-2 text-sm font-bold text-slate-500 bg-slate-100 rounded-xl hover:bg-blue-50 hover:text-blue-600 transition active:scale-95">+150</button>
                            <button type="button" onclick="setAmount(300)" class="px-4 py-2 text-sm font-bold text-slate-500 bg-slate-100 rounded-xl hover:bg-blue-50 hover:text-blue-600 transition active:scale-95">+300</button>
                            <button type="button" onclick="setAmount(500)" class="px-4 py-2 text-sm font-bold text-slate-500 bg-slate-100 rounded-xl hover:bg-blue-50 hover:text-blue-600 transition active:scale-95">+500</button>
                        </div>
                        ...
                        <div class="space-y-3">
                            ...
                            <button type="button" onclick="closeManualModal()" class="w-full py-3 rounded-xl text-slate-400 font-bold hover:bg-slate-50 hover:text-slate-600 transition">
                                Cancel
                            </button>
                        </div>
                    </form>
